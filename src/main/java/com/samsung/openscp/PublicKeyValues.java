...
import static com.samsung.openscp.ByteUtils.intToLength;
...
public abstract class PublicKeyValues {
    ...
    /**
     * Returns key in its encoded form
     */
     ...
     */
    abstract PublicKey toPublicKey() throws NoSuchAlgorithmException, InvalidKeySpecException;

    /**
     * Constructs a PublicKeyValues instance using values from a JCA {@link PublicKey}.
     *
     * @param publicKey the public key to extract values from
     * @return public key values
     */
    ...
    static class Cv25519 extends PublicKeyValues {
        private final EllipticCurveValues ellipticCurveValues;
        private final byte[] bytes;

        Cv25519(EllipticCurveValues ellipticCurveValues, byte[] bytes) {
        ...
        public String toString() {
            return "PublicKeyValues.Cv25519{" +
                    ...
                    ", publicKey=" + ByteUtils.bytesToHex(bytes) +
                    ", bitLength=" + bitLength +
                    '}';
        }
    }
    ...
    /**
     * Public key values for an elliptic curve key
     */
    public static class Ec extends PublicKeyValues {
        ...
        Ec(EllipticCurveValues ellipticCurveValues, BigInteger x, BigInteger y) {
            super(ellipticCurveValues.getBitLength());
            if (ellipticCurveValues == EllipticCurveValues.Ed25519 || ellipticCurveValues == EllipticCurveValues.X25519) {
                throw new IllegalArgumentException("InvalidCurve");
            }
        ...
        static Ec fromEncodedPoint(EllipticCurveValues curve, byte[] encoded) {
    ...
    static class Rsa extends PublicKeyValues {
        private final BigInteger modulus;
        private final BigInteger publicExponent;

        Rsa(BigInteger modulus, BigInteger publicExponent) {
